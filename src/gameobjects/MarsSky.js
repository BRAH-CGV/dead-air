...
const DEFAULT_MOONS = {
  // Deliberately enormous next to the real thing (Phobos is ~0.2° wide from
  // Mars, this is ~4°). The wrongness is the point — it reads as a world
  // that has been moved rather than one you recognise.
  phobos: { azimuth: -26, elevation:  8, size: 14, color: 0xb9a893 },
  deimos: { azimuth:  24, elevation: 12, size:  6, color: 0x9a8f83 },
};
 ...
 * @param {Object} [opts.phobos]  `{ azimuth, elevation, size, color }`, degrees.
...
function createMoon(name, { azimuth, elevation, size, color }, domeRadius) {
  ...
  mesh.name = name;
  mesh.position.copy(directionFromAngles(azimuth, elevation))
...
const DOME_FRAGMENT_SHADER = /* glsl */`
  ...
  void main() {
    ...
    float noise    = hash(floor(vWorldDir * 40.0));
    float star     = step(uStarThreshold, noise);
    // Offsetting the phase by the star's own hash stops them pulsing in unison.
    float twinkle  = 0.6 + 0.4 * sin(uTime * 2.0 + noise * 62.83);
    float horizonFade = smoothstep(0.0, 0.2, height);

    color += vec3(star * twinkle * horizonFade * uStarBrightness);
